...
export const createCartItem = async (req, res) => {
    try {
        const { cart, product, quantity, price } = req.body;
        if (!cart || !product || !quantity || price === undefined) {
            return res.status(400).json({ message: "cart, product, quantity and price are required" });
        }

        const existing = await CartItem.findOne({ cart, product });
        if (existing) {
        ...
        const item = new CartItem({ cart, product, quantity, price });
